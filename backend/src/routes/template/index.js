...
import { programmeWorkout } from "../../../drizzle/schema";
...
templateRoutes.post('/updateRecord', async (c) => {
    ...
    const data = await db
        ...
        .where(and(eq(programmeDetailsTable.exercise_id, body.exercise_id), eq(programmeDetailsTable.programme_workoutId, body.template_id)))
        .returning();
    return c.json(data);
});
