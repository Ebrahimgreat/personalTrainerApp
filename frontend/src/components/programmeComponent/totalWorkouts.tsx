    ...
    <div class="bg-white p-4 shadow-md">
        ...
        <ul class=" rounded list-decimal divide-y ">
            <For each={props.items}>
                {(item,key)=>(<li class={`group flex items-center justify-between px-4 py-2 cursor-pointer transition-colors rounded-md ${key()===props.workoutSignal?'bg-pink-700 text-white font-bold shadow-inner':'hover:bg-gray-100'}`} onclick={()=>props.setWorkoutSignal(key())}>
